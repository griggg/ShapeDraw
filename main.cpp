...
public:
  ...
  MyView(QGraphicsScene *scene, MyStorage *storage, QWidget *parent = nullptr)
  ...
  void setItemMode(ItemType mode) { this->itemMode = mode; }
...
public:
  ...
  MainWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
    ...
    connect(selectColor, &QPushButton::clicked,
            [this]() { this->view->color = QColorDialog::getColor(); });

    // fileLabel = new QLabel("Текущий файл: Нет", this);

    QPushButton *loadFromFile = new QPushButton("Открыть", this);
    connect(loadFromFile, &QPushButton::clicked, [this]() {
    ...
    connect(saveToFile, &QPushButton::clicked, [this]() {
      QString filename = QFileDialog::getSaveFileName(
          ...
          tr("Текстовые файлы (*.txt);;Все файлы (*.*)"));
      if (!filename
               .isEmpty()) { // Проверяем, что пользователь не отменил диалог
        storage->filename = filename.toStdString();
        storage->save();
    ...
    for (const auto &[text, type] : createButtonsInfo) {
      ...
      connect(creatingCircles, &QPushButton::clicked,
              [this, type] { this->view->setItemMode(type); });
