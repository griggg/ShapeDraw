...
public:
  ...
  virtual IShape *loadFromFile(ifstream &inFile) override {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (newItem == nullptr)
        throw runtime_error(
            "Error: GroupComposite::loadFromFile() newItem = nullptr");
  ...
  bool ismovable() override { return this->movable; }
...
public:
  ...
  CustomShape(int posX, int posY, int size, QGraphicsView *view,
  ...
  void saveToFile(ofstream &outFile) override {
    ...
    outFile << this->color.toRgb().red() << " " << this->color.toRgb().green()
  ...
  QRectF getRect() { return boundingRect(); }
  ...
  QPointF getMapFromScene(QPointF pos) { return this->mapFromScene(pos); }
  bool isContains(QPointF p) override {
...
public:
  ...
  QRectF boundingRect() const override {
    qreal currentPenW = pen().widthF();
    qreal padding = currentPenW / 2.0 + 2.0;
    return QRectF(-padding, -size / 2.0 - 1, 2 * padding, size + 2);
  }

  virtual void changeSize(QRectF rect) override { this->size = rect.height(); }
  ...
  void showUnSelected(QColor color) override { this->setPen(QPen(color, 5)); }
  IShape *loadFromFile(ifstream &inFile) override {
    int r, g, b;
    inFile >> r >> g >> b;
    int x, y, size;
...
public:
  ...
  void setInFile(ifstream &inFile) { this->inFile = &inFile; }
  static IShape *shapeByStr(ItemType type, QGraphicsView *view,
                            const QPointF &position, int size) {
    IShape *shape = nullptr;
    if (type == ItemType::CRCL) {
      shape = new MyCircleItem(position.toPoint().x(), position.toPoint().y(),
                               size, view);
    } else if (type == ItemType::SQUARE) {
      shape = new MySquareItem(position.toPoint().x(), position.toPoint().y(),
                               size, view);
    } else if (type == ItemType::LINE) {
      shape = new MyLineItem(position.toPoint().x(), position.toPoint().y(),
                             size, view);
    } else if (type == ItemType::TRPZD) {
      shape = new MyTrapezoidItem(position.toPoint().x(),
                                  position.toPoint().y(), size, view);
    } else if (type == ItemType::TRNGL) {
      shape = new MyTriangleItem(position.toPoint().x(), position.toPoint().y(),
                                 size, view);
